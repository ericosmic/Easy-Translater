...
async function translateText(text, settings, batchMode = false) {
  ...
  if (!apiKey && provider !== 'ollama') throw new Error('请先配置 API Key');
  ...
  switch (provider) {
    ...
    case 'ollama': return translateWithOllama(apiKey, apiUrl, model, systemPrompt, text);
...
async function translateWithOllama(apiKey, apiUrl, model, systemPrompt, text) {
  const url = `${apiUrl.replace(/\/$/, '')}/chat/completions`;
  let res;
  try {
    res = await fetch(url, {
    ...
    throw new Error('无法连接 Ollama，请确认 Ollama 已启动（ollama serve）');
  }
  if (res.status === 403) {
    throw new Error('Ollama 拒绝了扩展请求（CORS），请在启动 Ollama 时设置环境变量：OLLAMA_ORIGINS=*');
  }
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`API 错误 (${res.status}): ${err}`);
